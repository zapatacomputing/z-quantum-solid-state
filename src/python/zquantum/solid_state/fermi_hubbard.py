...
def calculate_exact_density_of_energy_for_infinite_1D_fermi_hubbard(tunneling_energy, coulomb_interaction_energy):
    """
    ...
    return integrate.quad(integrand,0,np.inf,args=(tunneling_energy, coulomb_interaction_energy))[0]

def get_fermi_hubbard_hamiltonian(x_dimension, y_dimension, tunneling, coulomb,
    chemical_potential=0.0, magnetic_field=0.0, periodic=True, spinless=False,
    ...
    """
    hamiltonian = fermi_hubbard(x_dimension, y_dimension, tunneling, coulomb,
    chemical_potential=chemical_potential, magnetic_field=magnetic_field,
    periodic = periodic, spinless = spinless, particle_hole_symmetry = particle_hole_symmetry)

    return get_interaction_operator(hamiltonian)

def get_fermi_hubbard_ordering(x_dimension, y_dimension, qpu_x_dimension, qpu_y_dimension,
