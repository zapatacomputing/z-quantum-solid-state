...
class TestFermiHubbard(unittest.TestCase):
    ...
    def test_get_fermi_hubbard_hamiltonian(self):
        ...
        # When
        test_interaction_op = get_fermi_hubbard_hamiltonian(
            2, 2, 1.0, 4.0, chemical_potential=0.5, spinless=True
        )
    ...
    def test_get_fermi_hubbard_ordering(self):
        # Given/When
        horizontal_ordering = get_fermi_hubbard_ordering(
            2, 2, 2, 4, ordering_type="sycamore-horizontal"
        )
        vertical_ordering = get_fermi_hubbard_ordering(
            2, 2, 2, 4, ordering_type="sycamore-vertical"
        )
        # Then
        ...
        # Given/When
        horizontal_ordering = get_fermi_hubbard_ordering(
            2, 2, 2, 4, ordering_type="sycamore-horizontal", initial_x=1, initial_y=1
        )
        vertical_ordering = get_fermi_hubbard_ordering(
            2, 2, 2, 4, ordering_type="sycamore-vertical", initial_x=1, initial_y=1
        )
        # Then
        ...
        # Given/When
        interleaved_ordering = get_fermi_hubbard_ordering(
            3, 3, 4, 5, ordering_type="sycamore-interleaved"
        )
        # Then
        self.assertEqual(
            interleaved_ordering,
            [0, 1, 2, 3, 10, 11, 8, 9, 16, 17, 18, 19, 14, 15, 12, 13, 4, 5],
        )

        # Given/When
        interleaved_ordering = get_fermi_hubbard_ordering(
            3, 4, 7, 4, ordering_type="sycamore-interleaved"
        )
        # Then
        self.assertEqual(
            interleaved_ordering,
